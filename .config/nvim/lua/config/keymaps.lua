...
map("n", ";d", "<cmd>bdelete<cr>", { desc = "Delete Buffer" })
...
map("n", ";j", function()
    ...
    -- local cmd = string.format("javac %s && java -cp %s %s\n", file, dir, class)
...
map("n", "<leader>se", "<C-w>=v", { desc = "Make splits equal size" })
map("n", "<leader>so", "<C-w>v<cmd>Telescope find_files<cr>", { desc = "Make splits equal size" })
...
-- better indenting
map("v", "<Tab>", ">gv", { desc = "Indent visual selection right" })
map("v", "<S-Tab>", "<gv", { desc = "Indent visual selection left" })
